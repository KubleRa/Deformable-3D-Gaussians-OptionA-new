...
def readNeuManSceneInfo(path, images, eval, llffhold=8):
    ...
    human_xyz, human_rgb = _load_smpl_canonical_vertices(path)
    if human_xyz is None or len(human_xyz) == 0:
        print("[Option A] No SMPL canonical vertices found "
              "(expected at smpl_pred/00000_png.npz) — falling back to "
        ...
        print(f"[Option A] Seeded {len(human_xyz)} human points from "
              f"SMPL canonical vertices (smpl_pred/00000_png.npz).")
    ...
    # Persist the merged cloud alongside the COLMAP one so `Scene` can dump
    # it as input.ply for reproducibility. The .ply itself does not carry the
    # is_human tag — it is kept in memory and re-derived on reloads.
...
sceneLoadTypeCallbacks = {
    ...
    "NeuMan": readNeuManSceneInfo,  # Changed — Option A: NeuMan reader with human-seed backprojection
}
